...
git tag v1.0.0 && git push origin v1.0.0
 ...
 GitHub Actions (.github/workflows/release.yml)
  ...
  ├─ Homebrew tap の Casks/cmdkana.rb を自動更新
...
| `TAP_GITHUB_TOKEN`             | tap リポジトリへ push できる Fine-grained PAT | ステップ E |
...
2. tap へ push できる **Fine-grained Personal Access Token** を作成
   - GitHub → Settings → Developer settings → **Personal access tokens → Fine-grained tokens** → Generate new token
   - **Repository access**: Only select repositories → `masakichi41/homebrew-tap`
   - **Permissions**: Repository permissions → **Contents: Read and write**
   - 有効期限は任意（切れたら再発行して Secret を更新）
3. 作成したトークンを本リポジトリの Secret **`TAP_GITHUB_TOKEN`** に登録

`Casks/cmdkana.rb` はリリース時に CI が [`dist/cmdkana.rb`](dist/cmdkana.rb) を雛形に `version` / `sha256` を差し込んで自動 push するため、手動で作成する必要はありません（`TAP_GITHUB_TOKEN` 未設定の間はこのステップだけ自動スキップされます）。
...
2. 表示された公開鍵を `CmdKana/Info.plist` の `SUPublicEDKey` に貼り付けます（設定済み。鍵を作り直した場合のみ更新）。
...
> ⚠️ `Info.plist` の `SUPublicEDKey` と Secret `SPARKLE_ED_PRIVATE_KEY` は同じ鍵ペアである必要があります。対になっていないと、配布したアプリは更新の署名を検証できません。
...
3. GitHub Actions の **Release** ワークフローが自動で走り、以下がすべて自動で行われます（数分かかります。Actions タブで進捗を確認）:
   - 署名・公証済み DMG を添付した Release の作成
   - **Homebrew tap の `Casks/cmdkana.rb` 更新**（ステップ E 設定後に有効。未設定の間は自動スキップ）
   - **Sparkle appcast の生成と `gh-pages` への公開**（ステップ F・G 設定後に有効。未設定の間は自動スキップ）
4. 動作確認:
...
> Cask の内容（説明文や zap 対象など）を変えたいときは、tap 側ではなく本リポジトリの `dist/cmdkana.rb` を編集してください。次のリリースで tap へ反映されます。
